...
const handler = async (req, res) => {
  ...
  if (req.method === 'GET') {
    try {
      const entity = await Category.findOne({ slug }).exec()
    ...
    switch (req.method) {
      case 'PUT': {
        ...
        try {
          ...
          const existingEntity = await Category.findOne({ slug, creator: existingUser._id }).exec()
          if (!existingEntity) {
      ...
      case 'DELETE': {
        try {
          ...
          // OPTION 1: remove()
          // Validate entity - entity must exist
          const existingEntity = await Category.findOne({ slug, creator: existingUser._id })
          if (!existingEntity) {
            return res.status(422).json({ status: false, message: 'Resource was not found' })
          }

          await Category.remove(existingEntity)

          // OPTION 2: findOneAndRemove()
          // const removedEntity = Category.findOneAndRemove({ slug, creator: existingUser._id })
          // if (!removedEntity) {
